...
const MAIL_FILE = path.join(DATA_DIR, 'mailaccounts.json');
...
let settings = Object.assign({ location: '北京', lat: 39.9042, lon: 116.4074, weatherUnit: 'c' }, readJSON(SETTINGS_FILE, {}));
let mailAccounts = readJSON(MAIL_FILE, { accounts: [] });

const sessions = new Map();
(function loadSessions() {
  const data = readJSON(SESSIONS_FILE, { sessions: [] });
  const now = Date.now();
  for (const s of data.sessions || []) { if (s && s.token && s.expiresAt > now) sessions.set(s.token, s); }
})();
function persistSessions() { writeJSON(SESSIONS_FILE, { sessions: Array.from(sessions.values()) }); }
...
function verifyPassword(password, stored) { if (!stored || !stored.includes(':')) return false; const [salt, hash] = stored.split(':'); const test = crypto.scryptSync(String(password), salt, 64); const ref = Buffer.from(hash, 'hex'); return test.length === ref.length && crypto.timingSafeEqual(test, ref); }
function uid() { return crypto.randomBytes(8).toString('hex'); }
function token() { return crypto.randomBytes(32).toString('hex'); }

function createSession(userId) { const t = token(); sessions.set(t, { token: t, userId, expiresAt: Date.now() + SESSION_TTL_MS }); persistSessions(); return t; }
function getSession(req) { const cookie = parseCookies(req.headers.cookie || ''); const t = cookie.sid; if (!t) return null; const s = sessions.get(t); if (!s) return null; if (s.expiresAt < Date.now()) { sessions.delete(t); persistSessions(); return null; } const user = users.users.find((u) => u.id === s.userId) || null; return user ? { user, token: t } : null; }
function destroySession(req) { const cookie = parseCookies(req.headers.cookie || ''); if (cookie.sid && sessions.has(cookie.sid)) { sessions.delete(cookie.sid); persistSessions(); } }
function parseCookies(header) { const out = {}; for (const part of header.split(';')) { const i = part.indexOf('='); if (i > -1) out[part.slice(0, i).trim()] = decodeURIComponent(part.slice(i + 1).trim()); } return out; }
...
function readBody(req) { return new Promise((resolve, reject) => { let size = 0; const chunks = []; req.on('data', (c) => { size += c.length; if (size > MAX_BODY) { reject(new Error('请求体过大')); req.destroy(); return; } chunks.push(c); }); req.on('end', () => resolve(Buffer.concat(chunks))); req.on('error', reject); }); }

const weatherCache = { data: null, key: '', at: 0 };
async function fetchWithTimeout(url, ms = 8000) { const ctrl = new AbortController(); const timer = setTimeout(() => ctrl.abort(), ms); try { const res = await fetch(url, { signal: ctrl.signal, headers: { 'User-Agent': 'xCloud/1.0' } }); if (!res.ok) throw new Error('upstream ' + res.status); return await res.json(); } finally { clearTimeout(timer); } }
async function geocode(city) { const url = `https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(city)}&count=1&language=zh&format=json`; const data = await fetchWithTimeout(url); const r = data && data.results && data.results[0]; if (!r) return null; return { location: r.name || city, lat: r.latitude, lon: r.longitude, country: r.country }; }
async function getWeather() { const key = `${settings.lat},${settings.lon},${settings.weatherUnit}`; if (weatherCache.data && weatherCache.key === key && Date.now() - weatherCache.at < WEATHER_CACHE_MS) return weatherCache.data; const units = settings.weatherUnit === 'f' ? 'fahrenheit' : 'celsius'; const url = `https://api.open-meteo.com/v1/forecast?latitude=${settings.lat}&longitude=${settings.lon}` + `&current=temperature_2m,relative_humidity_2m,apparent_temperature,weather_code,wind_speed_10m,is_day` + `&daily=weather_code,temperature_2m_max,temperature_2m_min&timezone=auto&forecast_days=6` + `&temperature_unit=${units}&wind_speed_unit=kmh`; const data = await fetchWithTimeout(url); const result = { location: settings.location, unit: settings.weatherUnit, current: data.current, daily: data.daily }; weatherCache.data = result; weatherCache.key = key; weatherCache.at = Date.now(); return result; }

const MIME_EXT = { 'image/png': 'png', 'image/jpeg': 'jpg', 'image/webp': 'webp', 'image/gif': 'gif', 'image/svg+xml': 'svg' };
function saveDataURL(dataUrl, prefix) { const m = /^data:(image\/[a-zA-Z+]+);base64,(.+)$/.exec(dataUrl || ''); if (!m) return { error: '无效的图片数据' }; const mime = m[1]; const ext = MIME_EXT[mime]; if (!ext) return { error: '不支持的图片格式' }; let buf; try { buf = Buffer.from(m[2], 'base64'); } catch { return { error: '图片解码失败' }; } if (buf.length > 3 * 1024 * 1024) return { error: '图片过大（≤3MB）' }; const name = `${prefix}-${Date.now()}-${uid()}.${ext}`; fs.writeFileSync(path.join(UPLOAD_DIR, name), buf); return { name }; }
function deleteUpload(name) { if (!name || typeof name !== 'string') return; const p = path.join(UPLOAD_DIR, path.basename(name)); try { if (fs.existsSync(p)) fs.unlinkSync(p); } catch { /* ignore */ } }

function requireAuth(req, res) { const s = getSession(req); if (!s) { sendError(res, 401, '未登录'); return null; } return s; }
...
function mimeOf(name) { return FILE_MIME[extOf(name)] || 'application/octet-stream'; }
function resolveFilePath(rel) { const base = path.resolve(FILES_DIR); const p = path.resolve(base, '.' + path.normalize('/' + String(rel || '').replace(/\\/g, '/'))); const rel2 = path.relative(base, p); if (rel2 === '') return base; if (rel2.startsWith('..') || path.isAbsolute(rel2)) return null; return p; }
function uniqueFilePath(p) { if (!fs.existsSync(p)) return p; const ext = path.extname(p); const base = p.slice(0, p.length - ext.length); let i = 1; while (fs.existsSync(`${base} (${i})${ext}`)) i++; return `${base} (${i})${ext}`; }
...
function listDir(rel) {
  const dir = resolveFilePath(rel); if (!dir) return { error: '路径无效' }; if (!fs.existsSync(dir)) return { error: '目录不存在' };
  const st = fs.statSync(dir); if (!st.isDirectory()) return { error: '不是目录' };
  const items = [];
  for (const name of fs.readdirSync(dir)) {
    const full = path.join(dir, name); let s; try { s = fs.statSync(full); } catch { continue; }
    const isDir = s.isDirectory();
    items.push({ name, type: isDir ? 'dir' : 'file', size: isDir ? 0 : s.size, sizeText: isDir ? '—' : formatSize(s.size), mtime: s.mtimeMs, ext: isDir ? '' : extOf(name), editable: isDir ? false : !!EDITABLE[extOf(name)], documentType: EDITABLE[extOf(name)] || null });
  }
  items.sort((a, b) => (a.type === b.type ? a.name.localeCompare(b.name, 'zh') : a.type === 'dir' ? -1 : 1));
  return { path: rel || '/', parent: parentPath(rel), items };
}
...
function handleUpload(req) {
  return new Promise((resolve) => {
    ...
    let targetDir = '', finalPath = null, error = null, settled = false, sawFile = false;
    const done = (r) => { if (!settled) { settled = true; resolve(r); } };
    bb.on('field', (name, val) => { if (name === 'path') targetDir = String(val || ''); });
    bb.on('file', (name, file, info) => {
      sawFile = true;
      const dir = resolveFilePath(targetDir); const fname = path.basename(decodeFilename(String(info.filename || '')));
      ...
      ws.on('finish', () => { let size = 0; try { size = fs.statSync(finalPath).size; } catch { /* ignore */ } done({ name: path.basename(finalPath), size }); });
      file.on('error', () => { error = '上传中断'; });
      file.pipe(ws);
    });
    bb.on('error', () => { error = error || '上传失败'; done({ error }); });
    bb.on('close', () => {
      if (!sawFile || !finalPath) { if (finalPath) { try { fs.unlinkSync(finalPath); } catch { /* ignore */ } } done({ error: error || '未收到文件' }); }
      else if (!settled) { setTimeout(() => { if (!settled) { try { done({ name: path.basename(finalPath), size: fs.statSync(finalPath).size }); } catch { done({ error: '上传失败' }); } } }, 100); }
    });
    req.pipe(bb);
  });
}
...
function signJWT(payload) { const h = b64url(JSON.stringify({ alg: 'HS256', typ: 'JWT' })); const p = b64url(JSON.stringify(payload)); const sig = crypto.createHmac('sha256', JWT_SECRET).update(`${h}.${p}`).digest('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, ''); return `${h}.${p}.${sig}`; }
function verifyJWT(t) { if (!t || typeof t !== 'string') return null; const parts = t.split('.'); if (parts.length !== 3) return null; const expected = crypto.createHmac('sha256', JWT_SECRET).update(`${parts[0]}.${parts[1]}`).digest('base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, ''); if (expected !== parts[2]) return null; try { const payload = JSON.parse(Buffer.from(parts[1], 'base64url').toString('utf8')); if (payload.exp && payload.exp < Date.now()) return null; return payload; } catch { return null; } }

function panelBaseUrl(req) { if (PANEL_URL) return PANEL_URL; const proto = (req.headers['x-forwarded-proto'] || 'http').split(',')[0].trim(); const host = req.headers['x-forwarded-host'] || req.headers.host || `localhost:${PORT}`; return `${proto}://${host}`; }

async function downloadFromOnlyOffice(url) {
  let target = url;
  if (ONLYOFFICE_INTERNAL_URL) { try { const u = new URL(url); const internal = new URL(ONLYOFFICE_INTERNAL_URL); if (u.host !== internal.host) { u.protocol = internal.protocol; u.host = internal.host; target = u.toString(); } } catch { /* keep original */ } }
...
function handleRaw(req, res, url) {
  const rel = url.searchParams.get('path') || ''; const payload = verifyJWT(url.searchParams.get('token'));
  if (!payload || payload.path !== rel) return sendError(res, 401, 'token 无效');
  const full = resolveFilePath(rel); if (!full || !fs.existsSync(full)) return sendError(res, 404, '文件不存在');
  const st = fs.statSync(full); if (st.isDirectory()) return sendError(res, 400, '是目录');
  const name = path.basename(full);
  res.writeHead(200, { 'Content-Type': mimeOf(name), 'Content-Length': st.size, 'Content-Disposition': `inline; filename*=UTF-8''${encodeURIComponent(name)}` });
...
async function handleCallback(req, res, url) {
  ...
  if (body.status === 2 || body.status === 6) {
    ...
    if (full && body.url) {
      try { const data = await downloadFromOnlyOffice(body.url); fs.writeFileSync(full, data); console.log('[onlyoffice] 已保存:', rel); }
      catch (e) { console.error('[onlyoffice] 保存失败:', e.message); return sendJSON(res, 200, { error: 1, message: 'save failed' }); }
    }
  }
...
function mailAccSafe(a) { return { id: a.id, name: a.name, email: a.email, imapHost: a.imapHost, imapPort: a.imapPort, imapSecure: !!a.imapSecure, smtpHost: a.smtpHost, smtpPort: a.smtpPort, smtpSecure: !!a.smtpSecure }; }
function mailFind(id) { return mailAccounts.accounts.find((a) => a.id === id) || null; }
function parseHostPort(s, defPort) { const str = String(s || '').trim(); if (!str) return null; const m = str.match(/^(.*?)(?::(\d+))?$/); return { host: m[1], port: m[2] ? Number(m[2]) : defPort }; }

function buildImapClient(acc, opts) {
  const hp = parseHostPort(acc.imapHost, acc.imapSecure ? 993 : 143);
  return new ImapFlow({ host: hp.host, port: hp.port, secure: !!acc.imapSecure, auth: { user: acc.email, pass: acc.password }, logger: false, ...(opts || {}) });
}

async function mailTest(acc) {
  const out = { imap: false, smtp: false, error: '' };
  try { const c = buildImapClient(acc, { verifyOnly: true }); await c.connect(); await c.logout(); out.imap = true; }
  catch (e) { out.error = 'IMAP: ' + (e.message || e); }
  try {
    const hp = parseHostPort(acc.smtpHost, acc.smtpSecure ? 465 : 587);
    const t = nodemailer.createTransport({ host: hp.host, port: hp.port, secure: !!acc.smtpSecure, auth: { user: acc.email, pass: acc.password } });
    await t.verify(); out.smtp = true;
  } catch (e) { out.error = (out.error ? out.error + '；' : '') + 'SMTP: ' + (e.message || e); }
  return out;
}

async function mailFolders(acc) {
  const c = buildImapClient(acc); await c.connect();
  try {
    const list = await c.list();
    const SPECIAL = { junk: [], trash: [], sent: [], draft: [] };
    const folders = list.filter((f) => !f.flags.includes('\\Noselect')).map((f) => {
    ...
    folders.sort((a, b) => { const rank = (p) => (p.toLowerCase() === 'inbox' ? 0 : 1); return rank(a.path) - rank(b.path); });
  ...
  try {
    ...
    try {
      ...
      for await (const msg of c.fetch(seq, { envelope: true, flags: true })) {
        ...
        items.push({
          uid: msg.uid, seq: msg.seq, flags: msg.flags || [],
...
async function mailRead(acc, folder, uid) {
  ...
  try {
    ...
    try {
      ...
      if (!(msg.flags || []).includes('\\Seen')) { try { await c.messageFlagsAdd(uid, ['\\Seen'], { uid: true }); } catch { /* ignore */ } }
      return {
        uid, flags: msg.flags || [],
        ...
        attachments: (parsed.attachments || []).map((a) => ({ filename: a.filename || '附件', contentType: a.contentType || '', size: a.size || 0 })),
      };
    } finally { lock.release(); }
  } finally { try { await c.logout(); } catch { /* ignore */ } }
}
...
async function mailSend(acc, body) {
  ...
  const mail = { from: `"${acc.name || acc.email}" <${acc.email}>`, to: toList, subject: String(body.subject || '(无主题)'), text: String(body.text || '') };
  if (ccList.length) mail.cc = ccList;
  if (bccList.length) mail.bcc = bccList;
  if (body.html) mail.html = String(body.html);
  if (Array.isArray(body.attachments)) {
    mail.attachments = body.attachments.map((a) => ({ filename: a.filename || 'attachment', content: Buffer.from(a.content || '', 'base64'), contentType: a.contentType || 'application/octet-stream' }));
  }
...
async function handleMailAPI(req, res, pathname, url, me) {
  if (pathname === '/api/mail/enabled' && req.method === 'GET') return sendJSON(res, 200, { enabled: mailEnabled(), accounts: mailAccounts.accounts.map(mailAccSafe) });
  if (pathname === '/api/mail/accounts' && req.method === 'GET') return sendJSON(res, 200, { accounts: mailAccounts.accounts.map(mailAccSafe) });
  if (pathname === '/api/mail/accounts' && req.method === 'POST') {
    const body = JSON.parse((await readBody(req)).toString('utf8') || '{}');
    const acc = { id: uid(), name: String(body.name || '').trim() || body.email, email: String(body.email || '').trim(), password: String(body.password || ''), imapHost: String(body.imapHost || '').trim(), imapPort: Number(body.imapPort) || 993, imapSecure: body.imapSecure !== false, smtpHost: String(body.smtpHost || '').trim(), smtpPort: Number(body.smtpPort) || 465, smtpSecure: body.smtpSecure !== false, createdAt: Date.now() };
    ...
    mailAccounts.accounts.push(acc); writeJSON(MAIL_FILE, mailAccounts); MAIL.lastUids.set(acc.id, 0);
    return sendJSON(res, 200, mailAccSafe(acc));
  }
  const accMatch = pathname.match(/^\/api\/mail\/accounts\/([a-f0-9]+)$/);
  if (accMatch && req.method === 'PUT') {
    const acc = mailFind(accMatch[1]); if (!acc) return sendError(res, 404, '账户不存在');
    const body = JSON.parse((await readBody(req)).toString('utf8') || '{}');
  ...
  if (accMatch && req.method === 'DELETE') {
    ...
    mailAccounts.accounts.splice(idx, 1); MAIL.lastUids.delete(accMatch[1]);
    writeJSON(MAIL_FILE, mailAccounts);
  ...
  if (pathname === '/api/mail/read' && req.method === 'GET') {
    ...
    try { const m = await mailRead(acc, folder, uid); if (!m) return sendError(res, 404, '邮件不存在'); return sendJSON(res, 200, m); }
  ...
  if (pathname === '/api/mail/attachment' && req.method === 'GET') {
    ...
    try {
      ...
      res.writeHead(200, { 'Content-Type': a.contentType, 'Content-Length': a.content.length, 'Content-Disposition': `attachment; filename*=UTF-8''${encodeURIComponent(a.filename)}` });
...
async function handleAPI(req, res, pathname, url) {
  if (pathname === '/api/status' && req.method === 'GET') {
    ...
    return sendJSON(res, 200, { initialized: users.users.length > 0, authenticated: !!s, onlyoffice: !!ONLYOFFICE_URL, user: s ? { id: s.user.id, username: s.user.username, name: s.user.name, avatar: s.user.avatar } : null });
  }
  if (pathname === '/api/setup' && req.method === 'POST') {
    if (users.users.length > 0) return sendError(res, 409, '系统已完成初始化');
    const body = JSON.parse((await readBody(req)).toString('utf8') || '{}');
    const username = String(body.username || '').trim(); const name = String(body.name || '').trim() || username; const password = String(body.password || '');
    ...
    users.users.push(user); writeJSON(USERS_FILE, users);
    const t = createSession(user.id); setCookie(res, t);
  ...
  if (pathname === '/api/login' && req.method === 'POST') {
    ...
    const key = username.toLowerCase(); await sleep(loginDelay(key));
    const user = users.users.find((u) => u.username === username);
    if (!user || !verifyPassword(password, user.passwordHash)) { failedLogins.set(key, (failedLogins.get(key) || 0) + 1); return sendError(res, 401, '用户名或密码错误'); }
    failedLogins.delete(key); const t = createSession(user.id); setCookie(res, t);
    return sendJSON(res, 200, publicUser(user));
  }
  if (pathname === '/api/logout' && req.method === 'POST') { destroySession(req); setCookie(res, '', 0); return sendJSON(res, 200, { ok: true }); }
  ...
  const s = requireAuth(req, res); if (!s) return; const me = s.user;
  ...
  if (pathname === '/api/profile' && req.method === 'POST') {
    ...
    if (body.name !== undefined) { const name = String(body.name || '').trim(); if (!name || name.length > 32) return sendError(res, 400, '名称需为 1–32 个字符'); stored.name = name; }
    if (body.avatar !== undefined) {
      if (body.avatar === null || body.avatar === '') { deleteUpload(stored.avatar); stored.avatar = ''; }
      else { const r = saveDataURL(body.avatar, 'avatar'); if (r.error) return sendError(res, 400, r.error); deleteUpload(stored.avatar); stored.avatar = r.name; }
    }
    writeJSON(USERS_FILE, users); return sendJSON(res, 200, publicUser(stored));
  }
  if (pathname === '/api/password' && req.method === 'POST') {
    const body = JSON.parse((await readBody(req)).toString('utf8') || '{}');
    const stored = users.users.find((u) => u.id === me.id);
    const cur = String(body.currentPassword || ''); const next = String(body.newPassword || '');
    ...
    stored.passwordHash = hashPassword(next); writeJSON(USERS_FILE, users);
    for (const [tk, sess] of sessions) { if (sess.userId === me.id && tk !== s.token) sessions.delete(tk); }
    persistSessions(); return sendJSON(res, 200, { ok: true });
  }
  if (pathname === '/api/apps' && req.method === 'GET') return sendJSON(res, 200, { apps: apps.apps.slice().sort((a, b) => (a.sort || 0) - (b.sort || 0)) });
  if (pathname === '/api/apps' && req.method === 'POST') {
    const body = JSON.parse((await readBody(req)).toString('utf8') || '{}');
    const app = validateApp(body); if (app.error) return sendError(res, 400, app.error);
    const next = { id: uid(), name: app.name, url_internal: app.url_internal, url_external: app.url_external, icon_type: app.icon_type, icon_value: app.icon_value, color: app.color, sort: apps.apps.length };
    apps.apps.push(next); writeJSON(APPS_FILE, apps); return sendJSON(res, 200, next);
  }
  const appMatch = pathname.match(/^\/api\/apps\/([a-f0-9]+)$/);
  if (appMatch && req.method === 'PUT') {
    const id = appMatch[1]; const idx = apps.apps.findIndex((a) => a.id === id);
    if (idx === -1) return sendError(res, 404, '应用不存在');
    const body = JSON.parse((await readBody(req)).toString('utf8') || '{}');
    const app = validateApp(body); if (app.error) return sendError(res, 400, app.error);
    const old = apps.apps[idx];
    apps.apps[idx] = Object.assign({}, old, { name: app.name, url_internal: app.url_internal, url_external: app.url_external, icon_type: app.icon_type, icon_value: app.icon_value, color: app.color });
    if (old.icon_type === 'image' && old.icon_value && old.icon_value !== apps.apps[idx].icon_value) deleteUpload(old.icon_value);
    writeJSON(APPS_FILE, apps); return sendJSON(res, 200, apps.apps[idx]);
  }
  if (appMatch && req.method === 'DELETE') {
    ...
    writeJSON(APPS_FILE, apps); return sendJSON(res, 200, { ok: true });
  }
  ...
  if (pathname === '/api/settings' && req.method === 'GET') return sendJSON(res, 200, { location: settings.location, weatherUnit: settings.weatherUnit });
  if (pathname === '/api/settings' && req.method === 'PUT') {
    ...
    if (body.location !== undefined) {
      ...
      if (typeof body.lat === 'number' && typeof body.lon === 'number') { settings.location = loc; settings.lat = body.lat; settings.lon = body.lon; }
      else if (loc !== settings.location) { const geo = await geocode(loc); if (!geo) return sendError(res, 400, '未找到该位置，请尝试城市名'); settings.location = geo.location; settings.lat = geo.lat; settings.lon = geo.lon; }
      weatherCache.at = 0;
    }
    writeJSON(SETTINGS_FILE, settings); return sendJSON(res, 200, { location: settings.location, weatherUnit: settings.weatherUnit });
  }
  ...
  if (pathname === '/api/files' && req.method === 'GET') {
    const rel = url.searchParams.get('path') || ''; const r = listDir(rel);
    if (r.error) return sendError(res, 400, r.error); return sendJSON(res, 200, r);
  }
  if (pathname === '/api/files/upload' && req.method === 'POST') {
    const r = await handleUpload(req); if (r.error) return sendError(res, 400, r.error); return sendJSON(res, 200, r);
  }
  if (pathname === '/api/files/download' && req.method === 'GET') {
    const rel = url.searchParams.get('path') || ''; const full = resolveFilePath(rel);
    if (!full || !fs.existsSync(full)) return sendError(res, 404, '文件不存在');
    const st = fs.statSync(full); if (st.isDirectory()) return sendError(res, 400, '是目录');
    const name = path.basename(full);
    res.writeHead(200, { 'Content-Type': mimeOf(name), 'Content-Length': st.size, 'Content-Disposition': `attachment; filename*=UTF-8''${encodeURIComponent(name)}` });
    return fs.createReadStream(full).pipe(res);
  }
  if (pathname === '/api/files/mkdir' && req.method === 'POST') {
    const body = JSON.parse((await readBody(req)).toString('utf8') || '{}');
    const parent = resolveFilePath(body.path || ''); const name = String(body.name || '').trim();
    ...
    fs.mkdirSync(target); return sendJSON(res, 200, { ok: true });
  }
  if (pathname === '/api/files/rename' && req.method === 'POST') {
    const body = JSON.parse((await readBody(req)).toString('utf8') || '{}');
    const full = resolveFilePath(body.path || ''); const name = String(body.name || '').trim();
    ...
    fs.renameSync(full, target); return sendJSON(res, 200, { ok: true });
  }
  if (pathname === '/api/files/move' && req.method === 'POST') {
    const body = JSON.parse((await readBody(req)).toString('utf8') || '{}');
    const from = resolveFilePath(body.from || ''); const toDir = resolveFilePath(body.to || '');
    ...
    fs.renameSync(from, target); return sendJSON(res, 200, { ok: true });
  }
  ...
  if (pathname === '/api/onlyoffice/config' && req.method === 'POST') {
    ...
    if (!full || !fs.existsSync(full)) return sendError(res, 404, '文件不存在');
    const st = fs.statSync(full); const name = path.basename(full); const ext = extOf(name); const docType = EDITABLE[ext];
    ...
    const config = {
      ...
      document: { fileType: ext, key, title: name, url: docUrl, permissions: { edit: true, download: true, print: true, review: true, comment: true } },
      editorConfig: { lang: 'zh-CN', mode: 'edit', callbackUrl: cbUrl, user: { id: me.id, name: me.name || me.username }, customization: { autosave: true, compactHeader: true, forcesave: true } },
      type: 'desktop',
    };
...
function validateApp(body) {
  const name = String(body.name || '').trim(); if (!name || name.length > 32) return { error: '名称需为 1–32 个字符' };
  const url_internal = cleanURL(body.url_internal); const url_external = cleanURL(body.url_external);
  ...
  if (icon_type === 'image') {
    if (/^data:image\//.test(icon_value)) { const r = saveDataURL(icon_value, 'icon'); if (r.error) return { error: r.error }; icon_value = r.name; }
...
function cleanURL(u) { const s = String(u || '').trim(); if (!s) return ''; return /^https?:\/\//i.test(s) ? s : `http://${s}`; }
function publicUser(u) { return { id: u.id, username: u.username, name: u.name, avatar: u.avatar }; }
function setCookie(res, value, maxAge = SESSION_TTL_MS / 1000) { const attrs = [`sid=${value || ''}`, 'Path=/', 'HttpOnly', 'SameSite=Lax', maxAge ? `Max-Age=${maxAge}` : 'Max-Age=0']; res.setHeader('Set-Cookie', attrs.join('; ')); }
...
function serveStatic(req, res, pathname) {
  let filePath;
  if (pathname.startsWith('/uploads/')) { filePath = path.join(UPLOAD_DIR, path.basename(pathname)); }
  else {
